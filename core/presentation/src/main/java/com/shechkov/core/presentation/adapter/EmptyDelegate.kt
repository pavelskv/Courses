...
import com.shechkov.core.presentation.databinding.ItemEmptyBinding
...
fun emptyDelegate() =
    adapterDelegateViewBinding<EmptyUi, ItemUi, ItemEmptyBinding>(
        { layoutInflater, root -> ItemEmptyBinding.inflate(layoutInflater, root, false) }
    ) {
        bind {
            binding.root.setText(item.messageResId)
        }
    }